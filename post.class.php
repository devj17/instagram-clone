<?php
  class post{
    ...
    public function getHomePost($way, $l, $count){
      ...
      if ($l == "nolimit") {
        $query = $this->db->prepare("SELECT post.post_id, post.user_id, post.type, post.time, post.font_size, post.address FROM post, follow_system WHERE follow_system.follow_by= :id AND post.user_id=follow_system.follow_to  AND post.post_of <> :grp 
        UNION SELECT post_id, user_id, type, time, font_size, address FROM post WHERE user_id= :id 
        ORDER BY time DESC LIMIT 10");
        $query->execute(array(":id" => $session, ":grp" => "group"));
        ...
        $query = $this->db->prepare("SELECT post.post_id, post.user_id, post.type, post.time, post.font_size, post.address FROM post, follow_system WHERE follow_system.follow_by = :id AND follow_system.follow_to = post.user_id AND post.post_of <> :grp AND post.post_id < :start
         UNION SELECT post_id, user_id, type, time, font_size, address FROM post WHERE user_id= :id 
        ORDER BY time DESC LIMIT 10");
        $query->execute(array(":id" => $session, ":grp" => "group", ":start" => $start));
      }
